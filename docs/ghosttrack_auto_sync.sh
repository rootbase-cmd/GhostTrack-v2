...
echo "[auto_sync] Inserimento menuPanel"
if ! grep -q 'id="menuPanel"' "$INDEX"; then
python3 - <<'PY'
import io,os
infile="docs/index.html"
with io.open(infile,"r",encoding="utf-8") as f:
    s=f.read()
block = """  <section id="menuPanel" class="panel" style="display:none;">
    ...
    <div class="card">
      ...
      <div class="video-wrap">
        <iframe width="100%" height="220" src="https://www.youtube.com/embed/live_stream?channel=UCakgsb0w7QB0VHdnCc-OVEA" frameborder="0" allowfullscreen></iframe>
      </div>
      <p class="muted">Embed YouTube live; sostituisci con il canale live desiderato.</p>
    </div>
  </section>"""
if 'id="menuPanel"' not in s:
