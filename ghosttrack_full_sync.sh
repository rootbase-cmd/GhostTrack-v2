...
PYEOF
  ...
  echo "🧪 Script ghosttrack_generate_all_panels.py già presente."
fi
echo

# 4) Genera tutti i pannelli (idempotente, non sovrascrive quelli esistenti)
echo "🛰 Generazione/aggiornamento pannelli da costellazione Wiki..."
python ghosttrack_generate_all_panels.py
echo "   ✔ Pannelli allineati in webapp/static/panels/"
echo

# 5) (Opzionale) Rigenera la UI se hai uno script dedicato
if [ -f build_webapp_ui.sh ]; then
  echo "🧱 Rigenerazione UI GhostTrack..."
  bash build_webapp_ui.sh
  echo "   ✔ UI rigenerata"
  echo
fi

# 6) Aggiungi i file principali e i nuovi pannelli a Git
echo "📦 Aggiunta file a Git..."
git add .gitignore || true
git add ghosttrack_generate_all_panels.py || true
git add ghosttrack_full_sync.sh || true
git add webapp/static/index.html 2>/dev/null || true
git add webapp/static/style.css 2>/dev/null || true
git add webapp/static/app.js 2>/dev/null || true
git add webapp/static/config.json 2>/dev/null || true
git add webapp/static/panels/ 2>/dev/null || true
git add diagnostica.sh 2>/dev/null || true
git add eco_launch.sh 2>/dev/null || true
git add render.yaml 2>/dev/null || true
git add README.md 2>/dev/null || true
git add api/app.py 2>/dev/null || true
git add api/requirements.txt 2>/dev/null || true
echo "   ✔ File aggiunti allo staging"
echo

# 7) Commit (se ci sono modifiche)
if git diff --cached --quiet; then
  echo "ℹ️ Nessuna modifica da committare."
else
  echo "📝 Creazione commit..."
  git commit -m "GhostTrack-v3: generazione pannelli costellazione + sync UI/API"
  echo "   ✔ Commit creato"
fi
echo
...
echo "✨ Ritual GhostTrack-v3 completato. Costellazione pannelli + UI/API ora allineati e pubblicati."
